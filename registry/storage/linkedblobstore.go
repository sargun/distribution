...
var (
	errNoDiscoveryMechanism = errors.New("No discovery mechanism for blob cross mount available")
	// This is purely used to terminate enumeration in a loop
	errBlobFound = errors.New("Blob Found")
)
...
// mount attempts to perform a cross-repo mount. If the mount is successful, the blob descriptor will be returned. If the
// mount was a "FromMount", then the reference returned should be non-nil.
